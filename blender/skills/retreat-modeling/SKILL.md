...
Use typed scene tools for focused edits. Use `bl_execute` for geometry construction and other bpy operations. The installed macOS launcher confines the modeling server and Blender to the configured modeling workspace, with network access blocked. Raw upstream/direct entry points do not supply isolation. Read `retreat_workflow` for this installation's workspace and isolation mode. Do not attempt to bypass restrictions or use external shell commands. Preserve unrelated geometry and use stable descriptive object names. Assign JSON-compatible data to `result`; Python local variables do not persist across calls. Blender datablocks do.

Before substantial edits, preserve a new revision with `retreat_save_revision` if needed. It creates a new file, never overwrites earlier revisions, and returns the absolute path. Use a project subfolder of the installation workspace, such as Backyard, Draculas Castle or Other Projects. Existing models and all linked textures/assets must be copied into that workspace before use. Access-denied errors outside it are expected; ask the user to copy the required files inside rather than weakening isolation. Do not place user models inside the plugin installation or source repository. The `.runtime` folder is reserved for isolated settings and temporary files.
